...
There is only a single test file in the directory. To run the file with verbosity use `pytest -v test_flask_api.py`. To run just the tests that concern the static html pages, use `pytest -v test_flask_api.py -m static`.
